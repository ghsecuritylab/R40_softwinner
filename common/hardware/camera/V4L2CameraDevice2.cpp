...
//workaroud  increase fps for cts
	if (strcmp(mCameraHardware->mCallingProcessName, "android.camera.cts") == 0 ||
		strcmp(mCameraHardware->mCallingProcessName, "com.android.cts.hardware") == 0 ) {
			...
			int cycle_duration_ms = 1000 / getFrameRate(); //ms
			int cycle_cnt = cycle_duration_ms / sleep_duration_ms + 1;
			int64_t used_duration_us;
			ALOGI("cts : duration set fps : %d, real fps : %d, cycle cnt : %d",mFrameRate,getFrameRate(),cycle_cnt);
			ALOGI("cts : sleep duration : %d ms,cycle duration : %d ms,cycle cnt : %d",sleep_duration_ms,cycle_duration_ms,cycle_cnt);
			for( int i = 0 ; i < cycle_cnt; i++) {
				pbuf->timeStamp = (int64_t)systemTime();
				mCallbackNotifier->onNextFrameAvailable((void*)pbuf, mUseHwEncoder);
				mPreviewWindow->onNextFrameAvailable((void*)pbuf);
				used_duration_us = ((int64_t)systemTime() - pbuf->timeStamp) / 1000;
				if (sleep_duration_ms*1000 > used_duration_us)
					usleep(sleep_duration_ms*1000 -used_duration_us);
			}
	} else {
		mCallbackNotifier->onNextFrameAvailable((void*)pbuf, mUseHwEncoder);
		// preview
...
int V4L2CameraDevice::openCameraDev(HALCameraInfo * halInfo)
{
	...
	LOGV("CAP.DRIVER IS :%s", cap.driver);
